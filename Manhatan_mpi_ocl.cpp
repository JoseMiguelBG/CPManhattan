...
void CodigoError(cl_int err)
{
	...
	case CL_PROFILING_INFO_NOT_AVAILABLE:
		...
		break;
	default:
		printf("C�digo de error no contemplado\n");
		break;
	}
}
...
void ocl(int N, int *A, int n, int *numeros, int *distancias, EntornoOCL_t entorno, int num_workitems, int workitems_por_workgroups)
{
	...
	buffA = clCreateBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, sizeof(int) * N * N, A, &error);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	buffNum = clCreateBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, sizeof(int) * n, numeros, &error);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	buffDis = clCreateBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, sizeof(int) * n, distancias, &error);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	buffPos = clCreateBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, sizeof(Matriz) * N * N * n, pos, &error);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	...
	error = clSetKernelArg(entorno.kernel, 1, sizeof(cl_mem), &buffA);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	error = clSetKernelArg(entorno.kernel, 2, sizeof(cl_mem), &buffNum);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	error = clSetKernelArg(entorno.kernel, 3, sizeof(cl_mem), &buffDis);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
	error = clSetKernelArg(entorno.kernel, 4, sizeof(cl_mem), &buffPos);
	if (error != CL_SUCCESS)
	{
		CodigoError(error);
	}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < cuantos; i++)
	{
		...
		int subn = n / size;
		int *subNum = (int *)malloc(sizeof(int) * subn);
		int *subDist = (int *)malloc(sizeof(int) * subn);

		// El proceso 0 debe repartir la informaci�n a procesar entre todos los procesos (incluido �l mismo)

		MPI_Scatter(numeros, subn, MPI_INT, subNum, subn, MPI_INT, 0, MPI_COMM_WORLD);

		ocl(N, A, subn, subNum, subDist, entorno, num_workitems, workitems_por_workgroups);
		...
		MPI_Gather(subDist, subn, MPI_INT, distancias, subn, MPI_INT, 0, MPI_COMM_WORLD);
